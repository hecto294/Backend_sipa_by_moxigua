...
class Settings:
    ...
    DEBUG: bool = _parse_bool_env("DEBUG", ENVIRONMENT != "production")
    APP_NAME: str = os.getenv("APP_NAME", "SIPA API")

    # Base de datos
    DATABASE_URL: str = os.getenv(
        "DATABASE_URL",
        "postgresql+psycopg2://postgres:admin123@localhost:5432/sipa_db?options=-c%20search_path=etapa_productiva"
    )
    ...
    PASSWORD_RESET_TOKEN_EXPIRE_MINUTES: int = _parse_int_env("PASSWORD_RESET_TOKEN_EXPIRE_MINUTES", 30)

    # CORS
    CORS_ORIGINS: List[str] = field(
        default_factory=lambda: _parse_cors_origins(
            os.getenv("CORS_ORIGINS", "http://localhost:5173,http://localhost:8000,http://127.0.0.1:8000")
        )
    )
    ...
    def __post_init__(self):
        """Validaciones posteriores a la carga de variables."""
    ...
    def _validar_cors(self):
        """En produccion no se permite '*'. En desarrollo se permite con advertencia."""
